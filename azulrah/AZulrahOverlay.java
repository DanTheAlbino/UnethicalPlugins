...
class AZulrahOverlay extends OverlayPanel {
   private static final Logger log = LoggerFactory.getLogger(AZulrahOverlay.class);
   ...
   @Inject
   private AZulrahOverlay(Client client, AZulrahPlugin plugin, AZulrahConfig config) {
      super(plugin);
   ...
   public Dimension render(Graphics2D graphics) {
      if (this.plugin.botTimer == null) {
         ...
         if (!tableComponent.isEmpty()) {
            ...
            this.panelComponent.getChildren().add(TitleComponent.builder().text("Anarchise Zulrah").color(ColorUtil.fromHex("#40C4FF")).build());
